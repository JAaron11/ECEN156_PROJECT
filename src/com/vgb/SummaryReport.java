...
public class SummaryReport {
	...
	public void printReport(List<Invoice> invoices, Map<String, Companies> companies) {
		...
		for (Invoice inv : invoices) {
			...
			String companyName = companies.containsKey(customerUUID) ? companies.get(customerUUID).getName() : customerUUID;
			System.out.printf("%-12s %-30s %-15d $%10.2f $%10.2f%n", 
					inv.getInvoiceId().toString(), companyName, numItems, tax, total);
		}
		System.out
				.println("+----------------------------------------------------------------------------------------+");
		System.out.printf("%-45s %-15d $%10.2f%n", "", overallItems, overallTax, overallTotal);
	}
